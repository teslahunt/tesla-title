...
test('Model S P85D', t => {
  ...
  t.is(
    title({ optionCodes: ['MDLS', 'PBT85', 'X024', 'DV4W'] }),
    'Model S P85D'
  )
})
...
test('Model S Long Range', t => {
  ...
  t.is(title({ optionCodes: ['MDLS', 'MTS10'] }), 'Model S Long Range')
})
...
test('Model S Plaid', t => {
  ...
  t.is(title({ optionCodes: ['MDLS', 'MTS12'] }), 'Model S Plaid')
  t.is(title({ optionCodes: ['MDLS', 'MTS14'] }), 'Model S Plaid')
})

test('Model S Plaid+', t => {
  t.is(title({ optionCodes: ['MDLS', 'MTS09'] }), 'Model S Plaid+')
})
